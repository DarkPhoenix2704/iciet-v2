const committe = [
    {
        role: 'Chief Patrons',
        members: [
            {
                name: 'Er. P H Muneer',
                role: 'Chairman, ICET',
            },
            {
                name: 'Mr. V U Sidhique',
                role: 'Manager, ICET',
            },
        ],
    },
    {
        role: 'Patrons',
        members: [
            {
                name: 'Dr. V H Abdul Salam',
                role: 'Director, ICET',
            },
            {
                name: 'Dr. K A Navas',
                role: 'Principal, ICET',
            },
        ],
    },
    {
        role: 'Conference General Chair',
        members: [
            {
                name: 'Dr. Abdul Gafur M',
                role: 'Professor(CSE)',
            },
        ],
    },
    {
        role: 'Conference General Co-Chair',
        members: [
            {
                name: 'Prof. Rosna P Haroon',
                role: 'Asst. Professor(CSE)',
            },
            {
                name: 'Prof. Ansar Jamal',
                role: 'Asst. Professor(EEE)',
            },
            {
                name: 'Dr. Sachin Gee Paul',
                role: 'Asso. Professor(EEE)',
            },
        ],
    },
    {
        role: 'Advisory Committee',
        members: [
            {
                name: 'Mr. K M Pareeth',
                role: 'Chairman, Ilahia Trust',
            },
            {
                name: 'Mr. M M Muhammed Kunju',
                role: 'Treasurer, Ilahia Trust',
            },
            {
                name: 'Mr. A A Rahim',
                role: 'Treasurer, ICET',
            },
            {
                name: "Dr. Faizal M H",
                role: "Vice Principal, ICET"
            },
            {
                name: 'Prof. Dr. K P Sudheer',
                role: 'Professor, IIT Madras',
            },
            {
                name: 'Dr. M P Sebastian',
                role: 'Professor, IIM Kozhikode',
            },
            {
                name: 'Dr. C H Abdul Buhari',
                role: 'Former Joint Director, Technical Education, Kerala',
            },
            {
                name: 'Mr. M A Mohammed Ali',
                role: 'Former Director, Cochin Oil Refinery',
            },
            {
                name: 'Mr. N Jehangir',
                role: 'Vice Chairman & MD, Nest Group of Companies',
            },
            {
                name: 'Mr. Navaz Meeran',
                role: 'Chairman, Eastern Group of Companies',
            },
            {
                name: 'Dr. Jaison Mathew',
                role: 'Asso. Prof. GEC Thrissur',
            },
            {
                name: "Dr. Mini Ulanat",
                role: "Section Chair, IEEE Kerala Section"
            },
            {
                name: "Dr. Chinmoy Saha",
                role: "Chairman, IEEE Microwave Theory and Technics Chapter, IEEE Kerala Section"
            },
            {
                name: "Dr. Biju K",
                role: "Treasurer, IEEE Kerala Section"
            }
        ],
    },
    {
        role: 'Technical Committee',
        members: [
            {
                name: 'Dr. Raman Menon Unnikrishnan',
                role: 'Professor, California State University, Fullerton, California',
            },
            {
                name: 'Dr. Tareq Alhmiedat',
                role: 'Head of Robotics & AI, University of Tabuk, Kingdom of Saudi Arabia',
            },
            {
                name: 'Dr. Vinod Pangracious',
                role: 'Asso. Prof. American University, Dubai',
            },
            {
                name: 'Dr. Azeemsha Thacham Poyil',
                role: 'Senior Embedded Software Engineer, Vectura Limited, United Kingdom',
            },
            {
                name: 'Dr. John Jose',
                role: 'Asso. Prof. IIT Guwahati',
            },
            {
                name: 'Dr. Unnikrishnan Cheramangalath',
                role: 'Asst. Prof., IIT Palakkad',
            },
            {
                name: 'Dr. S M Sameer',
                role: 'Professor, NIT Calicut',
            },
            {
                name: 'Dr. George K Varghese',
                role: 'Professor, NIT Calicut',
            },
            {
                name: 'Dr. Shobana N V',
                role: 'Professor, RIT Kottayam',
            },
            {
                name: "Dr. Anilkumar C V",
                role: "Professor, College of Engineering, Karunagappaly"
            },
            {
                name: "Er. Jithin Krishnan",
                role: "Scientist, SCTIMST, Thiruvananthapuram"
            },
            {
                name: "Dr. Isha T B",
                role: "Prof. Amrita Viswa Vidyapeetham, Coimbatore"
            },
            {
                name: "Dr. Jubilant Job",
                role: "Prof., Saintgits College of Engineering, Kottayam"
            },
            {
                name: "Dr. Barakkath Nisha U",
                role: "Asso. Prof., Sreekrishna College of Engineering, Coimbatore"
            },
            {
                name: "Dr. Varghese Paul",
                role: "Prof., Rajagiri School of Engineering & Technology, Ernakulam"
            },
            {
                name: "Dr. Janahanlal Stephen",
                role: "Retd. Prof., M A College of Engineering, Kothamangalam"
            },
            {
                name: 'Dr. Laju Kottallil',
                role: 'Professor, ICET',
            },
            {
                name: 'Dr. Benny Paul',
                role: 'Professor, ICET',
            },
            {
                name: 'Dr. Sujith Kumar P S',
                role: 'Professor, ICET',
            },
            {
                name: 'Dr. Vadhanakumari S',
                role: 'Professor, ICET',
            },
            {
                name: 'Dr. S Syed Ibrahim',
                role: 'Asso. Prof, ICET',
            },
            {
                name: 'Dr. Abhiraj T K',
                role: 'Asso. Prof, ICET',
            },
            {
                name: 'Dr. A Jahubar Ali',
                role: 'Professor, ICET',
            },
            {
                name: "Dr. Jinu Mohan", 
                role: "Asso. Prof., ICET"
            },
            {
                name: "Dr. P A Habeeb Rahiman",
                role: "Professor, ICET"
            },
            {
                name: "Dr. Shebin Sharief",
                role: "Asso. Prof., ICET"
            }
        ],
    },
];
export default committe;
